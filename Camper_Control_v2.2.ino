...
float R1 = 10003.0;  //R1 10K   (Actual measured value)
float R2 = 4690.0;   //R2 4.7K   (Actual measured value)
///Main Battery   A1
...
float mR1 = 10006.0;  //R1 10K    (Actual measured value)
float mR2 = 4678.0;   //R2 4.7K    (Actual measured value)
...
void setup() {
  ...
  pinMode(LPG_PIN, INPUT); //LPG Guage
...
void loop() {
  ...
  if (currentTime - prevT_lpgRead >= lpgRead_interval) { // Water Level
    prevT_lpgRead = millis();
    measure_lpg();
  }
...
void get_DCvolts() {
  ...
  if (mV_a >= 12.9) {                //Warning indicator for Main Batt  - Charge
    Serial2.print("v1.bco=1413");
    Serial2.write(0xff);
    Serial2.write(0xff);
    Serial2.write(0xff);
  }
  else if (mV_a <= 12.8 && mV_a >= 12.4) { //Warning indicator for Main Batt - Normal
  ...
  else if (mV_a <= 12.4 && mV_a >= 12.1) { //Warning indicator for Main Batt - Amber
    Serial2.print("v1.bco=64512");
    Serial2.write(0xff);
    Serial2.write(0xff);
    Serial2.write(0xff);
  }
  else if (mV_a <= 12.0) {  //Warning indicator for Main Batt - Red
...
void measure_lpg() {  //LPG Tank Guage
  ...
  {
    if (R2 >= 75) {
      Serial2.print(F("f0.txt=\""));  //Prints range text to text box on cab HMO
      Serial2.print("75-93");
      Serial2.print(F("\""));
      Serial2.write(0xff);
      Serial2.write(0xff);
